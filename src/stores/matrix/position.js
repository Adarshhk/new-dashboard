...
import { useManualPositionsStore } from "../groups/manualPosition"
...
export const usePositionsStore = defineStore("positions", () => {
  ...
  const getPositions = async () => {
    try {
      ...
      if (response.data) {
        ...
        mainPositions.value.map((pos, index) => ({
          ...pos,
    ...
    if (
      ...
      for (let i = 0; i < positions.value.length; i++) {
        const strgy = strategies.value.find(
          s => s.id === positions.value[i].strategy_id
        )
        const brkr = brokers.value.find(
          s => s.id === positions.value[i].broker_id
        )
  ...
  const getPositionsByUserId = async (id, subEndpoint) => {
    try {
  ...
  const getPositionsByStgId = async (id, subEndpoint) => {
    try {
      const response = await makeRequest(
        endpoint,
        "GET",
