...
export const useMatrixJoinersStore = defineStore("matrixJoiners", () => {
  ...
  watchEffect(() => {
    if (
      ...
      matrixJoiners.value.length > 0
    ) {
      for (let i = 0; i < matrixJoiners.value.length; i++) {
        const strgy = strategies.value.find(
          s => s.id === matrixJoiners.value[i].strategy_id
        )
        const brkr = brokers.value.find(
          s => s.id === matrixJoiners.value[i].broker_id
        )
  ...
  const getMatrixJoinersByUserId = async (id, subEndpoint) => {
    try {
