...
import { useStrategiesStore } from "./strategy"
import { useBrokersStore } from "./broker"
...
export const useOrdersStore = defineStore("orders", () => {
    ...
    data: [
      {
        ...
        status: "REJECTED",
        status_message:
          "Thank you for your order, placed on April 10, 2015. You will receive an email from us in 2-4 business days letting you know that your order has shipped..",
    ...
    if (
      ...
      for (let i = 0; i < orders.value.length; i++) {
        const strgy = strategies.value.find(
          s => s.id === orders.value[i].strategy_id
        )
        const brkr = brokers.value.find(s => s.id === orders.value[i].broker_id)
  ...
  const getOrdersByUserId = async (id, subEndpoint) => {
    try {
  ...
  const getMatrixOrdersById = async (id, subEndpoint) => {
    try {
      const response = await makeRequest(
        endpoint,
        "GET",
  ...
  async function addEditOrder(id, formdata) {
    try {
      if (id) {
