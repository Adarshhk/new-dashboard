...
export const useBrokersStore = defineStore("brokers", () => {
  ...
  const showOrderPositionModal = ref(false)
  const brokerOrders = ref([])
  const showPositions = ref(false)
  const brokerPositions = ref([])
  const showSqOffModal = ref(false)
  const brokerPosSqOffData = ref({})

  const showBrokerName = ref("")
  const storedBrokerData = ref({})
    ...
    data: [
      {
        ...
        broker_token:
          "eyJhbGciOiJIUzUxMiJ9..ZfsiyFJsUPU9CggVSVEijASvMaGJk7GxfWU7kfEW6MGOZIyncshYNvzP2YxBp5U1goEr9rxJICI5pa0pCyT1ow",
      ...
      },
      {
        ...
        broker_api:
          "ZCf46tl7zriTyCcxpteBtM8b2Q26VNs0QSgXOH8rvJHj8EvD2KI7crPiKkSq8aj2RZNRxXRoyQ5MTbhacD1Fxt9SzbpFL6ka0RCtu0IMZ6BRZk72tIPAWnRr0S2xi2Rn",
  ...
  const getBrokersById = id => {
    return brokers.value.find(broker => broker.id === id)
  }

  const getBrokersByUserId = async (id, subEndpoint) => {
    try {
  ...
  const showOrdersview = async broker => {
