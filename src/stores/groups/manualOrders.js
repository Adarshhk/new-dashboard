...
export const useManualOrdersStore = defineStore("manualOrders", () => {
  ...
  const totalManualOrders = ref(0) // for manualOrder
    ...
    try {
      ...
      if (response.data) {
        ...
        showEndRecords.value =
          manualOrders.value.length + (currentPageNumber.value - 1) * page_size
    ...
    if (brokers.value.length > 0 && manualOrders.value.length > 0) {
      for (let i = 0; i < manualOrders.value.length; i++) {
        const strgy = strategies.value.find(
          s => s.id === manualOrders.value[i].strategy_id
        )
        const brkr = brokers.value.find(
          s => s.id === manualOrders.value[i].broker_id
        )
