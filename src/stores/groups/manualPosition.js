...
import { useBrokersStore } from "../matrix/broker"
...
export const useManualPositionsStore = defineStore("manualPositions", () => {
  ...
  const mainManualPosition = ref({})

  const showManualSqOffModal = ref(false)
  const manualDataForSqOff = ref({ position_id: 0 })
  const mockdata = {
    status: "success",
  ...
  const getManualPositions = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        mainManualPosition.value.map((pos, index) => ({
          ...pos,
    ...
    if (brokers.value.length > 0 && manualPositions.value.length > 0) {
      for (let i = 0; i < manualPositions.value.length; i++) {
        const strgy = strategies.value.find(
          s => s.id === manualPositions.value[i].strategy_id
        )
        const brkr = brokers.value.find(
          s => s.id === manualPositions.value[i].broker_id
        )
